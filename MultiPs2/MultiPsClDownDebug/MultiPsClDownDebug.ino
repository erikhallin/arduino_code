...
void setup()
{
  ...
  for(int i=0;i<MAX_KEYBOARDS;i++)
  {
    ...
    //digitalWrite(g_data_pins[i],LOW);
...
void loop()
{
  ...
  for(int index_keyboard=0;index_keyboard<1;index_keyboard++)//MAX_KEYBOARDS
  {
...
{//Read scan code and send data to Serial
  ...
  for(int i=0;i<numof_chars;i++)
  {
    ...
    for(int time=0;time<time_out;time+=time_step)
    {
      if( digitalRead(g_clock_pins[index_keyboard])==HIGH )
      {
        lost_code=false;
        //Serial.println(time);
        break;
    ...
    //read DATA pin
    if( digitalRead( g_data_pins[index_keyboard] )==HIGH )
    {
      scan_code[i]='1';
    }
    else
